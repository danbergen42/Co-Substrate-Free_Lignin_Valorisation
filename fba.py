...
Key features:
- Acetate serves as control carbon source for growth without protocatechuate enforcement.
- p-Coumarate and ferulate simulations include *pcaHG* (PP_4655 and PP_4656) knockout 
  to prevent protocatechuate degradation.
...
with pd.ExcelWriter('fba/iJN1445_fba_results.xlsx') as writer:
    for substrate in fba_results.keys():
        ...
        model.reactions.get_by_id(uptake_reactions[substrate]).lower_bound = -4.5
